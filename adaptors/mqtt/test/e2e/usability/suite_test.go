...
func deployMQTTServer() {
	Expect(exec.RunKubectl(nil, GinkgoWriter, "apply", "-f", filepath.Join(testCurrDir, "test", "e2e", "usability", "mosquitto.yaml"))).
		Should(Succeed())
...
func isMQTTServerAvailable() {
	GinkgoT().Log("waiting MQTT server for ready")
